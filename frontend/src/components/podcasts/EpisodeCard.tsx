...
function useStatusMeta() {
  ...
  return {
    running: {
      label: t('podcasts.statusProcessing'),
    ...
    completed: {
      label: t('podcasts.statusCompleted'),
      className: 'bg-emerald-100 text-emerald-800 border-emerald-200',
    },
    failed: {
      label: t('podcasts.statusFailed'),
    ...
    pending: {
      label: t('podcasts.statusPending'),
    ...
    unknown: {
      label: t('podcasts.statusUnknown'),
      className: 'bg-muted text-muted-foreground border-transparent',
    },
  } as Record<EpisodeStatus | 'unknown', { label: string; className: string }>
}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {t('podcasts.profile')}: {episode.episode_profile?.name ?? t('podcasts.unknownProfile')}
              {createdLabel ? ` • ${t('podcasts.created')} ${createdLabel}` : ''}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
              ...
              <DialogContent className="w-[min(90vw,720px)] max-h-[85vh] overflow-hidden">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {episode.episode_profile?.name ?? t('podcasts.unknownProfile')}
                    {createdLabel ? ` • ${t('podcasts.created')} ${createdLabel}` : ''}
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 overflow-hidden">
                  ...
                  <Tabs defaultValue="summary" className="h-[60vh] flex flex-col">
                    <TabsList className="grid w-full grid-cols-3">
                      <TabsTrigger value="summary">{t('podcasts.summary')}</TabsTrigger>
                      <TabsTrigger value="outline">{t('podcasts.outline')}</TabsTrigger>
                      <TabsTrigger value="transcript">{t('podcasts.transcript')}</TabsTrigger>
                    </TabsList>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{t('podcasts.deleteEpisode')}</AlertDialogTitle>
                  <AlertDialogDescription>
